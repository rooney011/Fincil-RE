/**
 * Mem0 capstone — N=3 stability + confabulation + voice bleed. Mirrors
 * notes/agentmem-test/finish-test.mts exactly (same seed, same 3 recall queries,
 * same heuristics) so Mem0's numbers line up against AgentMem's Block L/J.
 *
 * Uses the default Mem0 write (infer:false → verbatim, synchronous) so the seed
 * memory is reliably present for the recall debates. Also tallies the per-debate
 * Mem0 op counts (1 search + 1 write) for the cost sheet.
 *
 * Run: PATH=/tmp/node-v20.18.1-linux-x64/bin:$PATH node --import tsx notes/mem0-test/finish-test.mts
 */
...
const USER_ID = `mem0-finish-${Date.now()}`;
const profile = {
...
async function runDebate(query: string, amount: number) {
  ...
  const r = await streamDebate(
    { query, amount, category: "electronics", profile, financeVerdict, relevantTransactions: [], activeGoals: [], userId: USER_ID,
      onRecall: (m: any) => { recallMs = m.ms; kept = m.keptCount; },
...
async function main() {
  console.log(`memoryProvider=${memoryProvider} memoryEnabled=${memoryEnabled} rerank=${process.env.MEM0_RERANK ?? "(default true)"} minRel=${process.env.MEM0_MIN_RELEVANCE ?? "(default 0)"}`);
  console.log(`user_id=${USER_ID}\n`);
  if (!memoryEnabled) { console.error("memoryEnabled FALSE — set MEM0_API_KEY. Aborting."); process.exit(1); }
  ...
  console.log(`\nMem0 ops: seed=1 write; each recall debate = 1 search + 1 write. ${runs.length} recall debates → ${runs.length} searches + ${runs.length + 1} writes total.`);

  writeFileSync("notes/mem0-test/finish-test-result.json", JSON.stringify({ ranAt: new Date().toISOString(), provider: memoryProvider, userId: USER_ID, memText, seed, runs,
  ...
  console.log(`\n✔ wrote notes/mem0-test/finish-test-result.json`);
}
